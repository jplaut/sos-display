...
vec3 map( vec3 p )
{
    ...
    p.xz += 0.5*cos( 2.0*ve*input_globalTime + (p.y+ph)*vec2(0.53,0.32) - vec2(1.57,0.0) );

    vec3 p1 = p; p1.xz += 0.15*sincos(p.y-ve*input_globalTime*ve+0.0);
    vec3 p2 = p; p2.xz += 0.15*sincos(p.y-ve*input_globalTime*ve+2.0);
    vec3 p3 = p; p3.xz += 0.15*sincos(p.y-ve*input_globalTime*ve+4.0);
...
vec3 render( in vec3 ro, in vec3 rd, in float px )
{
    ...
    if( res.x < maxdist )
    {
        ...
        float u = 800.0*res.z - sin(res.y)*input_globalTime;
        col *= 0.95 + 0.05*cos( u + 3.1416*cos(1.5*u + 3.1416*cos(3.0*u)) + vec3(0.0,1.0,2.0) );
        col *= vec3(1.5,1.0,0.7);
        col *= occ;

        float fl = mod( (0.5+cos(2.0+res.y*47.0))*input_globalTime + res.y*7.0, 4.0 )/4.0;
...
void main()
{
    vec2 p = (-input_resolution.xy+2.0*gl_FragCoord.xy)/input_resolution.y;
    ...
    vec3 col = render( ro, rd, 1.0/(input_resolution.y*fl) );
