...
vec4 map( in vec3 p )
{
    ...
    p = -4.0*p/dot(p,p);
    // twist space
    float an = -1.0*sin(0.1*input_globalTime + length(p.xz) + p.y);
    float co = cos(an);
    float si = sin(an);
    ...
    // pattern
    float f;
    vec3 q = p*0.85                     - vec3(0.0,1.0,0.0)*input_globalTime*0.12;
    f  = 0.50000*noise( q ); q = q*2.02 - vec3(0.0,1.0,0.0)*input_globalTime*0.12;
    f += 0.25000*noise( q ); q = q*2.03 - vec3(0.0,1.0,0.0)*input_globalTime*0.12;
    f += 0.12500*noise( q ); q = q*2.01 - vec3(0.0,1.0,0.0)*input_globalTime*0.12;
    f += 0.06250*noise( q ); q = q*2.02 - vec3(0.0,1.0,0.0)*input_globalTime*0.12;
    f += 0.04000*noise( q ); q = q*2.00 - vec3(0.0,1.0,0.0)*input_globalTime*0.12;
...
void main()
{
    ...
    // cameran
    //--------------------------------------
    float an = -0.07*input_globalTime + 3.0*mo.x;
    vec3 ro = 4.5*normalize(vec3(cos(an), 0.5, sin(an)));
    ro.y += 1.0;
    vec3 ta = vec3(0.0, 0.5, 0.0);
    float cr = -0.4*cos(0.02*input_globalTime);
    ...
    // dithering
    float t = 0.05*texture2D( input_channel0, gl_FragCoord.xy/input_channel0_resolution.x ).x;
    for( int i=0; i<128; i++ )
    {
        if( sum.a > 0.99 ) break;
        vec3 pos = ro + t*rd;
