...
float noise( in vec2 x )
{
    ...
    vec2 uv = (p.xy) + f.xy;
    return texture2D( input_channel0, (uv+ 0.5)/256.0, -100.0 ).x;
}
...
float noise( in vec3 x )
{
    ...
    vec2 rg = texture2D( input_channel0, (uv+ 0.5)/256.0, -100.0 ).yx;
...
vec3 raymarch( in vec3 ro, in vec3 rd, in vec2 ani, in vec2 pixel )
{
    ...
    // dithering
    float t = 0.03*texture2D( input_channel0, pixel.xy/input_channel0_resolution.x ).x;
...
void main()
{
    ...
    vec2 p = -1.0 + 2.0*q;
    p.x *= input_resolution.x/ input_resolution.y;

    float time = input_globalTime;
    ...
    // camera parameters
    vec4 camPars = texture2D( input_channel0, floor(1.0+input_globalTime/5.5)*vec2(5.0,7.0)/input_channel0_resolution.xy );

    // camera position
    vec3 ro = 4.0*normalize(vec3(cos(30.0*camPars.x + 0.023*time), 0.3+0.2*sin(30.0*camPars.x + 0.08*time), sin(30.0*camPars.x + 0.023*input_globalTime)));
    vec3 ta = vec3(0.0, 0.0, 0.0);
    float cr = 0.25*cos(30.0*camPars.y + 0.1*time);

    // shake
    ro += ani.x*ani.x*0.05*(-1.0+2.0*texture2D( input_channel0, 1.035*time*vec2(0.010,0.014) ).xyz);
    ta += ani.x*ani.x*0.20*(-1.0+2.0*texture2D( input_channel0, 1.035*time*vec2(0.013,0.008) ).xyz);
    ...
    // raymarch
    vec3 col = raymarch( ro, rd, ani, gl_FragCoord.xy );
    ...
    col *= 1.0-smoothstep( 0.4, 0.5, abs(fract(input_globalTime/5.5)-0.5) )*(1.0-sqrt(ani.x));
