...
float flame(vec3 p)
{
    ...
    return d + (noise(p+vec3(.0,input_globalTime*2.,.0)) + noise(p*3.)*.5)*.25*(p.y) ;
}
...
void main()
{
    vec2 v = -1.0 + 2.0 * gl_FragCoord.xy / input_resolution.xy;
    v.x *= input_resolution.x/input_resolution.y;
